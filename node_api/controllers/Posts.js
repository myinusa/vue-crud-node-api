  ...
  postObj
    ...
    .catch((err) => {
      result.status(500).send({
        message:
          err.message ||
          `Some error occurred while retrieving data with id : ${paramID}`,
      });
    });
};
...
exports.updatePostByID = (request, result) => {
  ...
  postObj
    ...
    .catch((err) => {
      result.status(500).send({
        message:
          err.message || `Error while updating Post object with id=${paramID}!`,
      });
    });
};
...
exports.deleteAllPosts = (request, result) => {
  postObj
    ...
    .catch((err) => {
      result.status(500).send({
        message:
          err.message || "Cannot delete Post objects. Something going wrong}!",
      });
    });
};
...
exports.getAllPublishedPosts = (request, result) => {
  postObj
    ...
    .catch((err) => {
      result.status(500).send({
        message:
          err.message || "Something going wrong. Unable to retrieve data!",
      });
    });
};
...
exports.getPostByTitle = (request, result) => {
  ...
  const condition = paramTitle
    ? { title: { [Op.like]: `%${paramTitle}%` } }
    : null;
